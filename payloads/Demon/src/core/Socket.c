...
PSOCKET_DATA SocketNew( SOCKET WinSock, DWORD Type, BOOL UseIpv4, DWORD IPv4, PBYTE IPv6, DWORD LclPort, DWORD FwdAddr, DWORD FwdPort, DWORD ParentID )
{
    ...
    if ( ( Type != SOCKET_TYPE_NONE ) && ( Type != SOCKET_TYPE_CLIENT ) )
    {
        ...
        else
        {
            ...
            /* Set bind address and port */
            memcpy( &SockAddr6.sin6_addr, IPv6, 16 );
    ...
    for ( ;; )
    {
        ...
        if ( Socket->Type == SOCKET_TYPE_CLIENT || Socket->Type == SOCKET_TYPE_REVERSE_PROXY )
        {
            ...
            do
            {
                 ...
                 */
                if ( Instance->Win32.ioctlsocket( Socket->Socket, FIONREAD, &bytesAvailable ) == SOCKET_ERROR )
                {
                ...
                // 将结果赋值回 PartialData.Length
                PartialData.Length = (UINT32)bytesAvailable;

                if ( PartialData.Length > 0 )
                {
                    PartialData.Buffer = MmHeapAlloc( PartialData.Length );
                    DWORD bytesRead = 0;  // 定义一个 DWORD 变量用于接收读取的数据长度
                    if ( ! RecvAll( Socket->Socket, PartialData.Buffer, PartialData.Length, &bytesRead ) ) {
                        Failed    = TRUE;
                        ErrorCode = Instance->Win32.WSAGetLastError();
                    }
                    PartialData.Length = (UINT32)bytesRead;
                    if ( PartialData.Length > 0 )
                    {
                        if ( ! FullData.Buffer )
                        {
                        ...
                        else
                        {
                            ...
                            memcpy( NewBuffer, FullData.Buffer, FullData.Length );
                            ...
                            NewBuffer = NULL;
                            // copy the new data
                            memcpy( C_PTR( U_PTR( FullData.Buffer ) + FullData.Length ), PartialData.Buffer, PartialData.Length );
                            FullData.Length += PartialData.Length;
                            // free the new data
...
PBYTE DnsQueryIPv6( LPSTR Domain )
{
    ...
    memcpy( IPv6, ((struct sockaddr_in6 *)res->ai_addr)->sin6_addr.u.Byte, 16 );
